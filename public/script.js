...
function run()
{
    ...
    $.getJSON("public/data.json", function(json)
    {
        ...
        for( objectwillchange in jsonObjectList){
            var str = jsonObjectList[objectwillchange].title;
            ...
            if( str.length > 15){
                ...
                if (str.indexOf(' ') === -1){
                    ...
                    jsonObjectList[objectwillchange].title = s1 + " " + s2;
                }
        ...
        for(let j=1; j<=20; j++)
        {
          ...
          var carouselText2 =[];
          for(let nItem in backgroundColors2)
          {
